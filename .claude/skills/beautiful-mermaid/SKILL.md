...
Render Mermaid diagrams to SVG or ASCII using the `beautiful-mermaid` npm package. Zero DOM dependencies, 15 built-in themes, works in Node.js/Bun without a browser.
...
5 types (use the existing `mermaid` skill's syntax references for full syntax):
...
For C4, Gantt, timeline, user journey, mindmap, pie, git graph — use the `mermaid` skill with `mmdc` instead.
...
## When to Use This vs `mermaid` Skill

| Need | Use |
|------|-----|
| SVG/ASCII for flowchart, sequence, class, state, ER | `beautiful-mermaid` |
| C4, Gantt, timeline, journey, mindmap, pie, git graph | `mermaid` (mmdc) |
| Terminal/ASCII output | `beautiful-mermaid` |
| Editor-themed diagrams (tokyo-night, dracula, etc.) | `beautiful-mermaid` |
| Server-side rendering without Puppeteer | `beautiful-mermaid` |
| Full Mermaid syntax compatibility | `mermaid` (mmdc) |
