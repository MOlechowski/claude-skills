...
`aws-local` is a drop-in replacement for `aws`:
...
AWS_ENDPOINT_URL=http://aws-localstack:4566 awslocal s3 ls
```
...
- Use `aws-localstack` skill to start/stop the LocalStack container
- Use `aws-localstack-expert` skill for testing strategies and CI/CD patterns
