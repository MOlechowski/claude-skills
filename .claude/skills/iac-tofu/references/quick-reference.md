...
```bash
tofu [global options] <command> [args]
```
...
tofu plan -var-file=staging.tfvars -out=plan.tfplan
tofu apply plan.tfplan
```
...
```bash
tofu plan -destroy -target=aws_instance.temp -out=destroy.tfplan
...
terraform {
  backend "s3" {
    bucket         = "my-tf-state"
    key            = "path/iac-terraform.tfstate"
    region         = "us-east-1"
