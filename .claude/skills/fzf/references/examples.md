...
fe() {
  ...
  file=$(fd --type f --hidden --exclude .git | \
    fzf --preview 'bat --color=always --style=numbers --line-range=:500 {}' \
        --preview-window=right:60% --height=100% --reverse \
...
fcd() {
  ...
  dir=$(fd --type d --hidden --exclude .git | \
    fzf --preview 'tree -C -L 2 {}' --preview-window=right:50% \
...
proj() {
  ...
  project=$(fd --max-depth 2 --type d . ~/Projects | \
    fzf --preview 'ls -la {}' --preview-window=up:40%) &&
  cd "$project" || return
}
...
fco() {
  ...
  branch=$(echo "$branches" | \
    fzf --preview 'git log --oneline --color=always $(echo {} | awk "{print \$1}")' \
        --preview-window=right:60% --header 'Checkout branch' \
        --bind 'ctrl-/:toggle-preview') &&
...
fcb() {
  ...
  base_branch=$(git branch | fzf --header 'Select base branch' | sed 's/^[* ]* //') &&
...
fshow() {
  git log --graph --color=always --format="%C(auto)%h%d %s %C(black)%C(bold)%cr" "$@" | \
    fzf --ansi --no-sort --reverse --tiebreak=index \
        ...
        --header 'Enter: show | Ctrl-/: toggle preview'
}
...
fcp() {
  ...
  commits=$(git log --oneline --color=always | \
    fzf --ansi --multi --preview 'git show --color=always {1}' \
...
fga() {
  ...
  files=$(git status -s | \
    fzf --multi --preview 'git diff --color=always {2}' \
        --preview-window=right:60% --header 'Select files to stage' \
        --bind 'ctrl-/:toggle-preview') &&
...
fgh() {
  ...
  git log --follow --oneline --color=always "$file" | \
    fzf --ansi --preview "git show --color=always {1}:$file" \
...
fkill() {
  local pid
  pid=$(ps aux | sed 1d | \
    fzf --multi --preview 'echo {} | awk "{print \$2}" | xargs ps -f -p' \
        --preview-window=down:40% --bind 'ctrl-r:reload(ps aux | sed 1d)' | \
    awk '{print $2}')
  [ -n "$pid" ] && echo "$pid" | xargs kill -9 && echo "Killed: $pid"
}
...
fptree() {
  ps aux | fzf --header-lines=1 --preview 'pstree -p {2}' --preview-window=right:60%
}
```
...
fdocker() {
  ...
  container=$(docker ps --format '{{.ID}} - {{.Names}} - {{.Image}}' | \
    fzf --header 'Select container' --preview 'docker logs --tail=100 {1}' \
...
fdlog() {
  local container
  container=$(docker ps --format '{{.ID}} - {{.Names}}' | fzf --header 'Select container') &&
...
fdrmi() {
  ...
  images=$(docker images --format '{{.ID}} - {{.Repository}}:{{.Tag}}' | \
    fzf --multi --header 'Select images to remove') &&
...
fdcup() {
  ...
  service=$(docker-compose config --services | \
    fzf --multi --header 'Select services') &&
...
fkexec() {
  local pod namespace
  pod=$(kubectl get pods --all-namespaces -o wide | \
    fzf --header-lines=1 --header 'Select pod') &&
...
fklogs() {
  local pod namespace
  pod=$(kubectl get pods --all-namespaces | fzf --header-lines=1) &&
  namespace=$(echo "$pod" | awk '{print $1}') &&
  pod=$(echo "$pod" | awk '{print $2}') &&
...
fkdel() {
  ...
  pods=$(kubectl get pods --all-namespaces | \
    fzf --multi --header-lines=1 --header 'Select pods to delete') &&
...
fenv() {
  local var
  var=$(env | sort | fzf --preview 'echo {}' --preview-window=down:3 \
...
fssh() {
  ...
  host=$(grep "^Host " ~/.ssh/config | awk '{print $2}' | \
    fzf --preview 'grep -A 10 "^Host {}" ~/.ssh/config' --header 'Select SSH host') &&
  ssh "$host"
}
...
ftmux() {
  ...
  session=$(tmux list-sessions -F '#{session_name}' 2>/dev/null | \
    fzf --header 'Select tmux session' --preview 'tmux capture-pane -pt {}') &&
...
fbrew() {
  local package
  package=$(brew search | fzf --preview 'brew info {}' --preview-window=right:60% \
...
fnpm() {
  ...
  package=$(npm search --json . 2>/dev/null | jq -r '.[].name' | \
    fzf --preview 'npm info {}' --header 'Select package') &&
  npm install "$package"
}
...
fpip() {
  ...
  package=$(pip list | tail -n +3 | awk '{print $1}' | \
    fzf --preview 'pip show {}' --header 'Installed packages') &&
  pip show "$package"
}
```
...
frename() {
  local files
  files=$(fd --type f | fzf --multi --preview 'bat --color=always {}' \
        --header 'Select files to rename')
  [ -z "$files" ] && return
  echo "$files" | while read file; do
    echo "Current: $file"
    read -p "New name: " newname
    [ -n "$newname" ] && mv "$file" "$newname" && echo "Renamed: $file -> $newname"
  done
}
...
fmv() {
  local files dest
  files=$(fd --type f | fzf --multi --header 'Select files to move')
  [ -z "$files" ] && return
  dest=$(fd --type d | fzf --header 'Select destination')
  [ -n "$dest" ] && echo "$files" | xargs -I {} mv {} "$dest" && echo "Moved to $dest"
}

# Duplicate finder
fdup() {
  fd --type f --exec md5sum {} \; | sort | awk '{print $1}' | uniq -d | \
    while read hash; do
      fd --type f --exec md5sum {} \; | grep "^$hash"
    done | fzf --multi --preview 'bat --color=always {2}' --header 'Duplicates'
}
```
...
fsr() {
  ...
  files=$(rg -l "$pattern" | fzf --multi --preview "rg --color=always '$pattern' {}" \
        --header 'Select files')
...
ftodo() {
  cat ~/todo.txt | fzf --preview 'echo {}' --header 'Select task' \
...
fbm() {
  ...
  local action=$(echo -e "Add\nOpen\nEdit" | fzf --header 'Bookmark manager')
  case "$action" in
    "Add")
      read -p "URL: " url; read -p "Title: " title
      echo "$url | $title" >> "$bookmarks" ;;
    "Open")
      url=$(cat "$bookmarks" | fzf --delimiter=' | ' --with-nth=2 --preview 'echo {1}') &&
      open "$(echo "$url" | awk -F' | ' '{print $1}')" ;;
    "Edit") $EDITOR "$bookmarks" ;;
  esac
}
```
