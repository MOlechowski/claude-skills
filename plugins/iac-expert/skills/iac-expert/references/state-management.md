...
terraform {
  backend "s3" {
    ...
    key            = "prod/networking/iac-terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
...
infrastructure/
├── networking/
│   ├── backend.tf    # s3://state/networking/iac-terraform.tfstate
│   └── main.tf
├── compute/
│   ├── backend.tf    # s3://state/compute/iac-terraform.tfstate
│   └── main.tf
└── database/
    ├── backend.tf    # s3://state/database/iac-terraform.tfstate
    └── main.tf
```
...
aws s3api get-object --bucket myorg-terraform-state --key prod/networking/iac-terraform.tfstate --version-id VERSION_ID recovered.tfstate
```
