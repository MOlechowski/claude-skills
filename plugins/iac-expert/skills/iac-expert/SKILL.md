...
- `iac-terraform` - Terraform CLI operations, HCL syntax, state commands
- `iac-tofu` - OpenTofu CLI operations, HCL syntax, state commands
- `aws-expert` - AWS service selection, Well-Architected patterns
- `cf-expert` - Cloudflare architecture, Zero Trust, Workers
...
```bash
python3 ~/.claude/skills/platform-architect/scripts/analyze_iac.py ./iac-terraform
```
...
```bash
python3 ~/.claude/skills/platform-architect/scripts/detect_antipatterns.py ./iac-terraform
```
