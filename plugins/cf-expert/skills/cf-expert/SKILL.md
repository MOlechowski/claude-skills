...
For CLI usage, see: `cf-tunnel`, `cf-wrangler`, `cf-ctl` skills.
...
Or use `/res-web Cloudflare pricing [service]` or check [cloudflare.com/plans](https://www.cloudflare.com/plans/).
...
Components:
1. Named tunnel for persistent URL (`cf-tunnel`)
2. WAF managed rules for baseline protection
3. Rate limiting ruleset to prevent abuse (OpenTofu)
4. DNS record pointing to tunnel (`cf-ctl` or OpenTofu)
...
Components:
1. Named tunnel to internal service (`cf-tunnel`)
...
3. Tunnel to protected API (`cf-tunnel`)
...
### Pricing (use /res-web for current rates)
