...
class TestSplitByH1:
    ...
    def test_h1_heading_included_in_text(self):
        text = "# My Chapter\n\nSome content here."
        chapters = split_by_h1(text)
    ...
    def test_h2_not_treated_as_split_point(self):
        text = "# Main\n\n## Sub\n\nSub content.\n\n## Sub 2\n\nMore content."
        chapters = split_by_h1(text)
        assert len(chapters) == 1, (
            f"Expected 1 chapter (H2 should not split), got {len(chapters)}"
        )
...
class TestSplitChapters:
    ...
    def test_unstructured_fallback_to_h1(self):
        # Unstructured text with H1 headings but no Chapter patterns → tries H1 as last resort
        text = "# Section A\n\nContent A.\n\n# Section B\n\nContent B."
        chapters = split_chapters(text, structured=False)
        assert len(chapters) == 2, (
            f"Expected 2 chapters from H1 fallback, got {len(chapters)}"
        )
